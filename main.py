...
# Danh sách User-Agent Chrome & Edge bản mới nhất (Win & Mac) để fake thiết bị
USER_AGENTS = [
    # Windows - Chrome
    ...
    # macOS - Safari (Cũng rất an toàn cho FB)
...
def apply_playwright_stealth(page):
    """
    Stealth tối giản: Chỉ bypass các cờ cơ bản nhất để không làm hỏng ReactJS của Facebook.
    Tuyệt đối không dùng thư viện playwright-stealth ngoài vì sẽ gây kẹt ở logo Meta.
    """
    try:
        # 1. Xóa cờ navigator.webdriver
        ...
        # 2. Fake ngôn ngữ tự nhiên (Tránh bị lộ là máy ảo không setup locale)
        page.add_init_script("""
        ...
        # 3. Fake plugin cơ bản để trông giống trình duyệt của người dùng thật
        page.add_init_script("""
        ...
        # 4. Xóa cờ automation mặc định của Chrome
        page.add_init_script("""
        ...
        print(f"Lỗi khi apply mini stealth: {str(e)}")
...
class FacebookCareTool(ctk.CTk):
    def __init__(self):
        ...
        self.logs = self.load_json(LOGS_FILE, [])
    ...
    def save_logs(self):
        self.save_json(LOGS_FILE, self.logs)
    ...
    def build_view_comment(self):
        ...
        ctk.CTkButton(right_panel, text="⏸ Tạm Dừng", height=40, fg_color="#475569").pack(fill="x", padx=20, pady=5)
    ...
    def start_comment_campaign(self):
        ...
        threading.Thread(
            ...
            args=(list(self.comment_selected_accounts), urls, raw_content), 
            daemon=True
        ).start()

    def run_comment_task(self, account_indexes, urls, raw_content):
        ...
        for i, url in enumerate(urls):
            acc_idx = account_indexes[i % len(account_indexes)]
            if acc_idx not in acc_tasks:
                acc_tasks[acc_idx] = []
    ...
    def normalize_cookie(self, cookie):
        ...
        if "expirationDate" in cookie: item["expires"] = int(cookie["expirationDate"])
        elif "expires" in cookie:
            try: item["expires"] = int(cookie["expires"])
            except: pass
        return item

    # ĐÃ CHỈNH SỬA: Sửa lại để không văng lỗi khi file cookie không tồn tại
    def load_cookies(self, account):
        cookie_file = account.get("cookie_file", "")
        if not cookie_file or not os.path.exists(cookie_file): 
            return [] # Trả về list rỗng để hàm login hiểu là chưa có cookie
        try:
            ...
            return [self.normalize_cookie(c) for c in cookies_to_process]
        except:
            return []

    def parse_proxy(self, proxy_text):
        proxy_text = (proxy_text or "").strip()
        if not proxy_text: return None
        if "://" not in proxy_text:
            parts = proxy_text.split(":")
            if len(parts) == 2: return {"server": f"http://{parts[0]}:{parts[1]}"}
            if len(parts) >= 4: return {"server": f"http://{parts[0]}:{parts[1]}", "username": parts[2], "password": ":".join(parts[3:])}
            raise Exception("Proxy không đúng định dạng.")
    ...
    def create_browser_page(self, p, cookies, account=None):
        ...
        launch_options = {"headless": False, "args": ["--disable-blink-features=AutomationControlled"]}
        ...
        browser = p.chromium.launch(
            ...
            args=[
                ...
                "--disable-blink-features=AutomationControlled" # Đảm bảo cờ này nằm trong args
            ]
        )
        
        # Bốc ngẫu nhiên 1 User-Agent từ danh sách
        random_ua = random.choice(USER_AGENTS)
        
        context = browser.new_context(
            viewport={"width": 1280, "height": 800},
            user_agent=random_ua # Gắn UA ngẫu nhiên vào đây
        )
        ...
        # Mở lại hàm stealth mini để tránh bị FB phát hiện bot
    ...
    def get_pause_seconds(self, pause_range):
        try:
            min_s, max_s = pause_range.split("-")
            return random.uniform(int(min_s), int(max_s))
        except: return random.uniform(4, 9)
    ...
    def scroll_page_for_minutes(self, page, minutes, pause_range, mode="newsfeed", account_name="", auto_like=False):
        ...
        while time.time() < end_time:
            ...
            time.sleep(self.get_pause_seconds(pause_range))
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                if settings["newsfeed_minutes"] > 0:
                    ...
                    self.scroll_page_for_minutes(page, settings["newsfeed_minutes"], settings["pause_range"], "newsfeed", account.get("name", ""))
                if settings["reels_minutes"] > 0:
                    page.goto("https://www.facebook.com/reel/", wait_until="domcontentloaded")
                    time.sleep(random.uniform(5, 8))
                    self.scroll_page_for_minutes(page, settings["reels_minutes"], settings["pause_range"], "reels", account.get("name", ""))
                if settings["newsfeed_minutes"] > 0:
                    page.goto("https://www.facebook.com/", wait_until="domcontentloaded")
                    time.sleep(random.uniform(5, 8))
                    # ---- TRUYỀN THÊM settings["auto_like"] ----
